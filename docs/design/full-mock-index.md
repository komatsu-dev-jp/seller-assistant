...
状態: M01〜M14・W01〜W12を承認済み。標準ホームはC「オーナーパルス」
...
| M13 | `mobile-13-solo-discrepancy-v2.png` | 1人運用の即時・可逆な紛失候補確認、商品/場所再読取、証拠写真、理由、復元、オフライン停止 |
| M14 | `mobile-14-accounting-readiness-v2.png` | 会計設定、承認済み勘定科目候補、用語ヘルプ、Money Forward/汎用CSV、停止理由、出力履歴 |
...
| W11 | `web-11-solo-discrepancy-control-v4.png` | 1人/複数人を自動判定する差異管制、証拠、復元、監査、システム場所、オフライン停止 |
| W12 | `web-12-accounting-profile-export-guard-v2.png` | 会計profile、勘定科目mapping、用語ヘルプ、出力前検査、Money Forward/汎用CSV、出力履歴 |
...
- M13 v2/W11 v4では、有効メンバー1人のときだけ同じ作業内で商品・場所再読取、証拠写真、理由、最終確認を行い、削除や廃棄ではなく戻せる紛失候補へ進める。2人以上は別担当確認を維持する。
- M14 v2/W12 v2では、会計設定と承認済み勘定科目ルールがそろわない出力を止め、Money Forward向けCSVと汎用CSVを分離する。外部APIや自動送信は行わない。

修正版4画面はSlackスレッド `1787203224.255009` で、ユーザー本人が2026-08-20 14:28:27 JSTに「修正版4画面で承認」と返信した。正確なハッシュとファイルIDは `revised-a-approval-v2.md` を参照する。
