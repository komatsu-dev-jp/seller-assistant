...
- 目的と参照: GitHub Pagesの承認済みモバイルUIで、架空商品1件の保管→検品→撮影→採寸を本人が操作し、再読み込み後も続きから確認できるようにする。正本は本書と`all-pages-interaction-plan.md`のP25。
- リスク: 中〜高。承認済み75画面と共有する大きなコンポーネント、ブラウザー保存、写真、画面遷移を扱う。ただしAPI、DB、実データは扱わない。
- 設計監査: 別実行の`gpt-6-astra` / low / `local_review_pwa_audit`が読み取り専用で既存画面、固定表示、保存境界、試験条件を監査済み。
- 実装担当: 別実行の`gpt-6-astra` / mediumを唯一のsource writerとする。ルート担当は計画・判断・統合・検証だけを行い、同じsourceファイルへ同時に書かない。
- 確認担当: 実装担当とは別実行の`gpt-6-astra` / mediumによる凍結差分の読み取り専用レビュー。ルート担当が自動試験と右サイドパネル390×844を確認する。
- 実装運転モード: `astra-centric`。保存初期化、写真Blob、画面間復元、入力検証が交差するためmediumとする。親セッションの実モデル、費用、利用枠は取得できず未確認・未計測。
- 変更可能: approved mobileの端末内確認用store/hook/photo helper、画面04／05・14〜28、対象CSS/test、reviewの正確な安全説明、計画・handoff・証拠。
- 変更禁止: API、DB、認証、実SKU、外部fetch、画像加工、OCR、出品・値下げ、会計、複数端末同期、承認済み画面順の変更、有料サービス、mainへの直接変更、PR merge、Pages公開。
- 受け入れ条件: P25チェックリスト、対象試験、review build、root check、git diff check、390×844の実ブラウザー、外部API request 0、独立レビューP1/P2 0件。写真保存失敗を成功表示にしない。
- 停止・昇格条件: 承認デザイン変更、実データ・外部接続、既存Web業務画面への影響、保存済みデータの無言破棄、原因不明の写真保存失敗を検出したら変更を広げずルートへ戻す。
- 実行結果: Astra medium writerが端末内の文字保存と写真保存を分離し、架空商品1件の保管→検品→撮影→採寸を画面04／05・14〜28へ実装した。初回独立レビューのP2 3件（写真token競合、reset部分失敗後の古いUI、04／05／17の承認構造回帰）と再レビューのP3 1件（検品6/6再開時の余分な操作）を修正した。
- 検証結果: 対象19テスト、最終`npm run check`（115ファイル・871テスト）、review production build（134ページ・137ファイル）、`git diff --check`をPASS。右サイドパネル390×844で保管、検品6項目、写真8枚、採寸4項目、2回の再読み込み復元、横はみ出し0、console error／warning 0を確認した。最終独立レビューはP1=0、P2=0、P3=0。
- 状態: P25の実装とPCブラウザー検証は完了。実iPhone Safariのカメラ／写真選択、ホーム画面起動、再読み込み復元はPages公開後の利用者確認として残し、Draft PR、merge、Pages公開は別承認まで停止する。
