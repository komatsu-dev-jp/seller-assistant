...
## 2026-09-24 公開モバイルでの再発・限定修正

利用者から10画面の操作不能が報告された。原因は見本の行がdivのまま、長押し説明が通常リンク、箱カウンターが未使用の真偽値だけを変更していたこと。見た目を操作可能・保存可能の根拠にできない問題として本記録へ追記し、重複incidentを作らない。

`docs/implementation/mobile-reported-controls.md`で範囲を固定し、対象画面と連続する会計2画面を確認版専用の操作へ置き換えた。実業務APIには接続しない。専用タブ内保存、画像の一時比較、失敗・未確認の表示を区別する。実装者と別実行Astra lowのレビューでCSV再試行問題を修正し、P1/P2 0件。実iPhoneと実ファイル保存先は未確認なのでcandidateを維持する。memory/INDEXへの昇格は行わない。
